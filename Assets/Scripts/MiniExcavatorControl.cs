...
namespace VehiclePhysics.Specialized
{
    ...
    public class MiniExcavatorControl : VehicleBehaviour
    {
        ...
        public VPVehicleJoint boomSwingJoint;
        public Transform boomSwingTransform;
        ...
        public float minBoomSwingAngle = -90.0f;
        public float maxBoomSwingAngle = 90.0f;
        ...
        public float boomSwingRate = 0.1f;
        ...
        [Range(0, 1)]
        public float startBoomSwingPosition = 0.5f;
        [Range(0, 1)]
        ...
        [Range(-1, 1)]
        public float boomSwingInput = 0.0f;
        [Range(-1, 1)]
        ...
        public float boomSwingPosition { get { return m_boomSwingPosition; } }
        ...
        float m_boomSwingPosition = 0.0f;
        ...
        public override void OnEnableVehicle()
        {
            ...
            m_boomSwingPosition = startBoomSwingPosition;
        ...
        public override void FixedUpdateVehicle()
        {
            ...
            m_boomSwingPosition = Mathf.Clamp01(m_boomSwingPosition + boomSwingInput * boomSwingRate * Time.deltaTime);
            ...
            // Update sideshift joint (motion in the X axis)
            ...
            if (boomSwingJoint != null)
            {
                float boomSwingAngle = Mathf.Lerp(minBoomSwingAngle, maxBoomSwingAngle, m_boomSwingPosition);
                boomSwingJoint.targetRotation = Quaternion.AngleAxis(-boomSwingAngle, Vector3.up) * boomSwingJoint.referenceRotation;

                // First run translates the rotation to the current position

                if (m_firstRun)
                    boomSwingJoint.transform.localRotation = Quaternion.AngleAxis(boomSwingAngle, Vector3.up);
            }
        ...
        void LateUpdate()
        {
            // Positional joints update the visual position in world coordinates


            // Rotational joints update the visual rotation in local coordinates

            UpdateAttachmentTransform(swingTransform, swingJoint);
            UpdateAttachmentTransform(boomSwingTransform, boomSwingJoint);
